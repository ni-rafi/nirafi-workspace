...
export const TimberBeamDesignDrawing: React.FC = () => {
  const width = 320;
  const height = 180;
    ...
    <div className="flex justify-center border border-border/30 bg-muted/5 rounded-2xl p-4 max-w-[450px] mx-auto w-full shadow-inner">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full aspect-[1.78] overflow-visible">
        ...
        <g>
          ...
          <text x={startX} y={beamY + 14} className="fill-muted-foreground text-[8px] font-mono" textAnchor="middle">A</text>
          ...
          <circle cx={endX + 2} cy={beamY + 7.5} r={1.2} className="fill-muted-foreground stroke-none" />
          <text x={endX} y={beamY + 14} className="fill-muted-foreground text-[8px] font-mono" textAnchor="middle">B</text>
          ...
          <text x={midX} y={beamY - 21} className="fill-rose-500 text-[8px] font-mono font-bold" textAnchor="middle">
            P
          </text>
          ...
          <path
            ...
            strokeWidth={0.8}
          />
          <text x={startX + 30} y={beamY - 14} className="fill-blue-500 text-[8px] font-mono" textAnchor="middle">
            w
          </text>

          {/* Span Dimension (20 ft) */}
          <line x1={startX} y1={beamY - 25} x2={endX} y2={beamY - 25} className="stroke-muted-foreground/30" strokeWidth={0.6} />
          <text x={midX} y={beamY - 28} className="fill-muted-foreground text-[8px] font-mono" textAnchor="middle">
            L = 20 ft
          </text>
        </g>

        {/* LEFT: Shear Force Diagram (SFD) */}
        <g>
          {/* Baseline */}
          <line x1={startX} y1={sfdY} x2={endX} y2={sfdY} className="stroke-border" strokeWidth={0.8} />
          
          {/* SFD Shape (Step jumps) */}
          {/* Left goes up to V1, slopes down to V2, jumps down due to P, slopes to V3 */}
          <path
            ...
            strokeWidth={1}
          />
          <text x={startX + 10} y={sfdY - 18} className="fill-blue-500 text-[7.5px] font-mono font-bold">
            +V_max
          </text>
          <text x={endX - 10} y={sfdY + 20} className="fill-blue-500 text-[7.5px] font-mono font-bold" textAnchor="end">
            -V_max
          </text>
          <text x={startX - 15} y={sfdY + 3.5} className="fill-muted-foreground text-[8px] font-mono font-bold">SFD</text>
        </g>

        {/* LEFT: Bending Moment Diagram (BMD) */}
        <g>
          {/* Baseline */}
          <line x1={startX} y1={bmdY} x2={endX} y2={bmdY} className="stroke-border" strokeWidth={0.8} />
          
          {/* BMD Shape (Parabolic curves meeting at midX) */}
          <path
            ...
            strokeWidth={1}
          />
          <text x={midX} y={bmdY - 29} className="fill-amber-500 text-[7.5px] font-mono font-bold" textAnchor="middle">
            M_max
          </text>
          <text x={startX - 15} y={bmdY + 3.5} className="fill-muted-foreground text-[8px] font-mono font-bold">BMD</text>
        </g>
        ...
        <g>
          ...
          <text x={257.5} y={32} className="fill-foreground text-[9.5px] font-mono font-bold" textAnchor="middle">
            10 in.
          </text>
          ...
          <text x={295} y={93.5} className="fill-amber-500 text-[10px] font-mono font-black" textAnchor="start">
            d = ?
          </text>

          {/* Material Label */}
          <text x={257.5} y={150} className="fill-muted-foreground text-[8px] font-mono font-bold" textAnchor="middle">
            Timber Grade
          </text>
        </g>
      </svg>
