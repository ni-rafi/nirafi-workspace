...
export const SlopeChart: React.FC<SlopeChartProps> = ({
  ...
  // Find max absolute slope value for scaling
  let maxAbsS = 1e-6;
  points.forEach((p: IDeflectionPoint) => {
    if (Math.abs(p.slope) > maxAbsS) {
      maxAbsS = Math.abs(p.slope);
    }
  });
  const scaleY = 55 / maxAbsS;
  ...
  if (hoverX !== null) {
    ...
    const projY = midY - closest.slope * scaleY;
    hoverData = {
      x: hoverX,
      slope: closest.slope,
      px: (projX / width) * 100,
      ...
      <div className="relative">
        ...
        >
          ...
          <defs>
            ...
            <linearGradient id="slopeGradNeg" x1="0%" y1="0%" x2="0%" y2="100%">
          ...
          {splitIntoSignSegments(points.map(p => ({ x: p.x, y: p.slope }))).map((seg, idx) => {
            const isPos = seg.isPos;
            const strokeColor = isPos ? 'var(--primary, #6366f1)' : '#f97316';
            const fillGradient = isPos ? 'url(#slopeGradPos)' : 'url(#slopeGradNeg)';
          ...
          {deflectionResult.criticalPoints.map((cp, idx) => {
            ...
            const py = midY - cp.slope * scaleY;
            const isAbove = cp.slope >= 0;
            const ptColor = cp.slope >= 0 ? 'var(--primary, #6366f1)' : '#f97316';
            return (
              <g key={idx}>
          ...
          {hoverData && (
            <g>
              <motion.line
                ...
                y2={height - 10}
                stroke={hoverData.slope >= 0 ? 'var(--primary, #6366f1)' : '#f97316'}
                strokeWidth={1}
                strokeDasharray="3,3"
              ...
              <motion.circle
                ...
                cy={midY - hoverData.slope * scaleY}
                r={4}
                fill={hoverData.slope >= 0 ? 'var(--primary, #6366f1)' : '#f97316'}
                transition={{ type: 'spring', stiffness: 260, damping: 30 }}
              />
            </g>
          )}
        ...
        <AnimatePresence>
          {hoverData && (
            ...
            >
              ...
              <div className="text-xs font-bold" style={{ color: hoverData.slope >= 0 ? 'var(--primary, #6366f1)' : '#f97316' }}>
                Slope: {hoverData.slope.toFixed(6)} rad
              </div>
            </motion.div>
          )}
        </AnimatePresence>
