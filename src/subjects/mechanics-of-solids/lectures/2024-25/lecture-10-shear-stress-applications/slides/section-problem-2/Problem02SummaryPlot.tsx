...
import { SlideParagraph, SlideTable, ParameterSlider } from '@/features/presentation/components/elements';
...
export const Problem02SummaryPlot: React.FC = () => {
  const [inspectY, setInspectY] = useUrlSyncedState<number>('problem2_inspect_y', 0);
        ...
        <div className="bg-muted/30 border border-border/50 rounded-xl p-4 flex flex-col items-center justify-center h-full min-h-[250px]">
          <svg viewBox={`0 0 ${svgW} ${svgH}`} className="w-full max-w-[270px] overflow-visible">
            <ProfileShapeView
              ...
              sliceDirection={inspectY >= 0 ? 'above' : 'below'}
            />
